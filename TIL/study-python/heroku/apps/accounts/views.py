...
def follow(request, pk):
    ...
    you = get_user_model().objects.get(id=pk)
    if request.user.followings.filter(id=you.id).exists():
        request.user.followings.remove(you)
        return redirect("accounts:detail", pk)
    else:
        if pk == my_pk:
            messages.warning(request, "자기 자신은 팔로우 할 수 없습니다.")
            return redirect("accounts:detail", pk)
        else:
            request.user.followings.add(you)
            messages.success(request, "목록에 추가되었습니다.")
            return redirect("accounts:detail", pk)
